...
cover_struct cover[DIMMER_COVER_NUM];
#endif // DIMMER_COVER_NUM

void DT_cover_init()
{
#if DIMMER_COVER_NUM > 0
    for (uint8_t num = 1; num < (DIMMER_COVER_NUM * 2); ++num)
    {
    ...
    for (uint8_t num = 0; num < (RELAY_COVER_NUM * 2); ++num)
    {
        DT_relay(num, false);
    }
    for (uint8_t num = DIMMER_COVER_NUM; num < (DIMMER_COVER_NUM + RELAY_COVER_NUM); ++num) // init variables
    {
        cover[num].step = cover_step_off;
        cover[num].pos = 0;
        cover[num].go_pos = 0;
...
void cover_write(uint8_t num, bool val)
{
#if DIMMER_COVER_NUM > 0
    if (num < DIMMER_COVER_NUM)
    {
        uint8_t pin = pgm_read_byte(OPT_ARRAY + DIMMER_LIGHT_NUM + DIMMER_HEAT_NUM + num);
...
#if RELAY_COVER_NUM > 0
    if (num > DIMMER_COVER_NUM && num < (DIMMER_COVER_NUM + RELAY_COVER_NUM))
    {
...
void cover_loop()
{
    ...
    for (uint8_t num = 0; num < DIMMER_COVER_NUM; ++num)
    {
        if (cover[num].step == cover_step_start) // demande de monté
        {
            ...
            {
                cover_write((num * 2) + 1, LOW); // arret de la descente
                // _NOP();
                cover_write(num * 2, HIGH); // marche de la montée
            ...
            {
                cover_write((num * 2), LOW); // arret de la monté
                // _NOP();
                cover_write((num * 2) + 1, HIGH); // marche de la descente
                cover[num].mouve_start = millis();
                cover[num].old_pos = cover[num].pos;
        ...
        else if (cover[num].step == cover_step_up) // Monté en cours
        {
            ...
            if (cover[num].pos == cover[num].go_pos)
            {
                ...
                {
                    cover_write((num * 2), LOW); // arret de la monté
                    cover[num].step = cover_step_none;
                }
            }
        }
